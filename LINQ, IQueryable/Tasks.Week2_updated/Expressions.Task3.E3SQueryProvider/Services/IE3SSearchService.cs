...
namespace Expressions.Task3.E3SQueryProvider.Services
{
    public interface IE3SSearchService
    {
      ...
      IEnumerable<T> SearchFts<T>(List<string> query, int start = 0, int limit = 0) where T : BaseE3SEntity;

      IEnumerable SearchFts(Type type, List<string> query, int start = 0, int limit = 0);
   }
}
